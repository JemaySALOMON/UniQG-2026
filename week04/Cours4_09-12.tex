...
\title[Génét. Quanti., UniQ ]{Introduction aux MLMs et egstimation de (co)variances génétiques}
...
\setbeamertemplate{itemize item}[ball]
    ...
    \item $R$ : matrice de dimension $n \times n$ de variance-covariance des erreurs (supposée définie positive, donc inversible); $R = \sigma_e^2 I_n$ où $I_n$ correspond à la matrice identité $n \times n$
\end{itemize}
}
\end{frame}
...
\begin{itemize}
    ...
    \item \textbf{paramètres} : $\Theta = \{\boldsymbol{\beta}, \sigma_e^2\}$
\end{itemize}

\vspace{0.5cm}
\[
\mathbf{y} = X\boldsymbol{\beta} + \boldsymbol{\epsilon} \quad\text{avec}\quad \boldsymbol{\epsilon} \overset{\text{iid}}{\sim} \mathcal{N_{iid}}(0, \sigma_e^2 I_n)
\]

\[
\Leftrightarrow \quad \mathbf{y} \mid X, \boldsymbol{\beta}, \sigma_e^2 \sim \mathcal{N}(X\boldsymbol{\beta}, \sigma_e^2 I_n)
\]
...
\begin{itemize}
    ...
    \item $\text{Cov}[\mathbf{y} \mid \sigma_e^2] = \sigma_e^2 I_n$
\end{itemize}
...
\[
\mathcal{L}(\Theta; \mathcal{D}) = f(\mathbf{y} \mid X, \boldsymbol{\beta}, \sigma_e^2) 
= \prod_{i=1}^n f(y_i \mid X_{i\cdot}, \boldsymbol{\beta}, \sigma_e^2)
\]
...
\[
= -\frac{n}{2}\log(2\pi) - \frac{n}{2}\log(\sigma_e^2) - \frac{1}{2\sigma_e^2}(\mathbf{y} - X\boldsymbol{\beta})^\top(\mathbf{y} - X\boldsymbol{\beta})
\]
...
\subsection{Estimation de $\sigma_e^2$}
%diapos
\begin{frame}
\frametitle{Estimation de $\sigma_e^2$ }
...
&= \text{tr}[(I_n - P)(X\boldsymbol{\beta}\boldsymbol{\beta}^\top X^\top + \sigma_e^2 I_n)] \\
&= \sigma_e^2 \text{tr}[(I_n - P)] \\
&= \sigma_e^2 (n - r(X))
\end{align*}
\end{block}
...
\[
\frac{\partial \ell}{\partial \sigma_e^2}(\hat{\sigma_e}^2) = 0 \quad\Leftrightarrow\quad 
\hat{\sigma_e}^2_{\text{ML}} = \frac{1}{n}(\mathbf{y} - X\hat{\boldsymbol{\beta}})^\top(\mathbf{y} - X\hat{\boldsymbol{\beta}}) = \frac{\text{RSS}}{n}
\]
\end{block}
...
\[
\mathbb{E}[\hat{\sigma_e}^2_{\text{ML}}] = \frac{n - r(X)}{n} \sigma_e^2
\]
L'estimateur ML est \textbf{biaisé} (sous-estime $\sigma_e^2$).
\end{block}
}
...
\begin{itemize}
    ...
    \item $\boldsymbol{\varphi}$ : vecteur des composantes de la variance, ici égal à $(\sigma_u^2, \sigma_e^2)^\top$
...
\]
avec $\mathbf{u} \sim \mathcal{N}(0, G)$, $\boldsymbol{\epsilon}  \overset{\text{iid}}{\sim} \mathcal{N}(0, R)$ et $\text{Cov}[\mathbf{u}, \boldsymbol{\epsilon}] = 0$
...
\begin{itemize}
    ...
    (égale ici à $\sigma_u^2 ZAZ^\top + \sigma_e^2 V$)
\end{itemize}
...
\begin{frame}
\frametitle{Estimation de $\sigma_u^2$ et $\sigma_e^2$}
...
\[
L(\boldsymbol{\varphi}; \mathbf{x}) \propto f(\mathbf{y}|\boldsymbol{\beta},\mathbf{u},\sigma_e^2) \times f(\mathbf{u}|\sigma_u^2)
\]
\[
\ell(\sigma_e^2) = -\frac{n}{2}\log 2\pi - \frac{n}{2}\log\sigma_e^2 - \frac{\boldsymbol{\epsilon}^\top\boldsymbol{\epsilon}}{2\sigma_e^2}
\]
\[
