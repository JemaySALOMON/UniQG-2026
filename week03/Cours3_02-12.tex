...
\begin{frame}
\frametitle{Modèle génétique à un locus et deux allèles (Modèle de Fisher)}
\only<1>{
...
\[
\begin{cases}
\text{Si l’allèle tiré est } A_{i'} \text{ ou } A_{j'} : P(i \equiv i') = 0 \\
\text{Si l’allèle tiré est } A_i \text{ ou } A_j : P(i \equiv i') = 1/2
\end{cases}
\]
...
Gamètes produits : $A_i$ et $A_j$ (avec probabilité $\frac{1}{2}$ chacun)

\medskip
\textbf{Tableau de Punnett :}
...
\medskip
\textbf{Probabilités génotypiques chez le descendant :}
\[
P(A_iA_i) = \frac{1}{4}, \quad
P(A_iA_j) = \frac{1}{2}, \quad
P(A_jA_j) = \frac{1}{4}
\]
}
...
\item Cette approche peut être étendue aux données génomiques pour construire automatiquement les matrices d’apparentement (VanRaden, 2008)
\end{itemize}
...
\begin{itemize}
\item Ameliore la précision sur les estimations des valeurs génétiques
\item Augmenter la puissance des analyses, par exemple en GWAS ou en sélection génomique. 
\end{itemize} 
\item \textbf{Flexibilité :} permet d’intégrer des pedigrees complexes ou des données génomiques pour mieux prendre en compte la structure génétique de la population
\end{itemize}
